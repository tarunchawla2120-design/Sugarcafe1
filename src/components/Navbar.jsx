...
const [showLocationModal, setShowLocationModal] = useState(false);
      ...
      async (currentUser) => {
        ...
        try {
          ...
          const profile = {
            uid: currentUser.uid,
            customerId,
            phone: currentUser.phoneNumber || existing.phone || "",
            name: existing.name || "Sugar Customer",
  ...
  useEffect(() => {
    ...
    const activeStatuses = ["New", "Preparing", "Food Ready", "Dispatched"];
...
/* =========================
  ...
  const handleProfileClick = () => {
    if (user && userData?.name && userData.name !== "Sugar Customer" && userData?.phone) {
      navigate("/profile");
    } else {
  ...
  <div className="location-info">
    ...
    <div className="location-address">
      {location.address}
        ...
        <button
          className={`profile-circle ${activeOrder ? "has-active-order" : ""}`}
          onClick={() =>
            activeOrder
              ? navigate("/orders")
              : handleProfileClick()
          ...
          title={
            activeOrder
              ? `Track Order #${activeOrder.orderNumber || activeOrder.id.slice(-6)}`
              : "Profile"
          }
        >
          {activeOrder ? (
            <>
              <FaMotorcycle />
              <span className="active-order-dot" aria-hidden="true" />
            </>
          ) : user ? (
            <FaUser />
          ) : (
        ...
        <div className="detect-box">
          ...
          <div className="detect-message">
            ...
            <div className="detect-text">
              ...
              <div>
                Give us your exact location for
              </div>
